...
y = tf.nn.softmax(tf.matmul(x, W) + b)#revering to prev batch
...
W_conv1 = weight_variable([5, 5, 1, 32])# conv layer with 32 filter #weight layer
b_conv1 = bias_variable([32])#filter 32
...
W_conv2 = weight_variable([5, 5, 32, 64])#conv layer with 32 filter
b_conv2 = bias_variable([64])#filter 32

h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2) + b_conv2)
h_pool2 = max_pool_2x2(h_conv2)

W_fc1 = weight_variable([7 * 7 * 64, 1024])#fully connected weight one#classification
...
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))# acccuracy built in by tensorflow
